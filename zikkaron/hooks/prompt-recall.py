...
Uses FTS5 keyword search + sqlite-vec vector search for hybrid retrieval.
Embedding model is loaded once and cached in-process.

Target latency: <300ms. Falls back to FTS5-only if vector search is slow.
"""
...
def _fts_search(conn, query: str, directory: str) -> list[dict]:
    ...
    try:
        rows = conn.execute(
            ...
            (fts_query, MIN_HEAT, MAX_RESULTS * 3),
        ).fetchall()
...
def _vector_search(conn, query: str, directory: str) -> list[dict]:
    ...
    try:
        import importlib
        # Quick check: skip if sentence_transformers isn't even importable
        ...
        for r in rows:
            if r[3] >= MIN_HEAT and r[4] != "":  # heat filter + has directory
                results.append({
                    ...
                    "directory": r[4], "score": 1.0 - r[1],  # distance to similarity
                    "source": "vec",
                })
        return results
    except Exception:
